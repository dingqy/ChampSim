...
public:
  ...
  const uint32_t NUM_SET, NUM_WAY, WQ_SIZE, RQ_SIZE, PQ_SIZE, MSHR_SIZE, INVALID_SIZE;
  const uint32_t HIT_LATENCY, FILL_LATENCY, OFFSET_BITS;
  std::vector<BLOCK> block{NUM_SET * NUM_WAY};
  const uint32_t MAX_READ, MAX_WRITE;
  uint32_t reads_available_this_cycle, writes_available_this_cycle, invalid_available_this_cycle;
  ...
  uint64_t RQ_ACCESS = 0, RQ_MERGED = 0, RQ_FULL = 0, RQ_TO_CACHE = 0, PQ_ACCESS = 0, PQ_MERGED = 0, PQ_FULL = 0, PQ_TO_CACHE = 0, WQ_ACCESS = 0, WQ_MERGED = 0,
           WQ_FULL = 0, WQ_FORWARD = 0, WQ_TO_CACHE = 0, IVQ_ACCESS = 0, IVQ_FULL = 0, IVQ_TO_CACHE = 0, IVQ_MERGED = 0;
...
#include "cache_modules.inc"
  ...
  // constructor
  // three new: invalid_size, cache_type, ul
  CACHE(std::string v1, double freq_scale, unsigned fill_level, uint32_t v2, int v3, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8, uint32_t hit_lat,
        ...
        unsigned pref_act_mask, MemoryRequestConsumer* ll, pref_t pref, repl_t repl, MemoryRequestConsumer* ul_0, MemoryRequestConsumer* ul_1,
        uint32_t invalid_size, uint32_t cache_type)
      : champsim::operable(freq_scale), MemoryRequestConsumer(fill_level), MemoryRequestProducer(ll, ul_0, ul_1), NAME(v1), NUM_SET(v2), NUM_WAY(v3),
        WQ_SIZE(v5), RQ_SIZE(v6), PQ_SIZE(v7), MSHR_SIZE(v8), INVALID_SIZE(invalid_size), HIT_LATENCY(hit_lat), FILL_LATENCY(fill_lat),
        OFFSET_BITS(offset_bits), MAX_READ(max_read), MAX_WRITE(max_write), prefetch_as_load(pref_load), match_offset_bits(wq_full_addr),
        virtual_prefetch(va_pref), pref_activate_mask(pref_act_mask), cache_type(cache_type), repl_type(repl), pref_type(pref)
  {
  }
};
