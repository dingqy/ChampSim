...
PageTableWalker::PageTableWalker(string v1, uint32_t cpu, unsigned fill_level, uint32_t v2, uint32_t v3, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7,
    ...
    : champsim::operable(1), MemoryRequestConsumer(fill_level), MemoryRequestProducer(ll, nullptr, nullptr), NAME(v1), cpu(cpu), MSHR_SIZE(v11), MAX_READ(v12),
...
int PageTableWalker::add_ivq(PACKET* packet) { return 0; }
